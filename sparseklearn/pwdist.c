...
int64_t func_U0(double *result, int64_t nrow, int64_t ncol, struct CONSTANTS *C, struct DATA *D,\
                int64_t *S, double *W, double *Sigma)\
{\
    int64_t rr, rc, dr, dc, q1, q2;\
...
int64_t func_U1(double *result, int64_t nrow, int64_t ncol, struct CONSTANTS *C, struct DATA *D,\
                int64_t *S, double *W, double *U, double *Sigma)\
{\
...
macro_U0(
    _U0_W0_Sig0_Pow1,
    result[ncol*rr+rc] += fabs(D->RHDX[(C->Q)*dr+q1] - D->RHDX[(C->Q)*dc+q1]);
)

macro_U0(
    _U0_W0_Sig0_Pow2,
    result[ncol*rr+rc] += \
        (D->RHDX[(C->Q)*dr+q1] - D->RHDX[(C->Q)*dc+q1]) * \
        (D->RHDX[(C->Q)*dr+q1] - D->RHDX[(C->Q)*dc+q1]);
)
            ...
            int power, int64_t *S, double *W, double *U, double *Sigma)
{
    if (!U && !W && !Sigma && power == 1)
        _U0_W0_Sig0_Pow1(result, nrow, ncol, C, D, S, W, Sigma);
    else if (!U && !W && !Sigma && power == 2)
        _U0_W0_Sig0_Pow2(result, nrow, ncol, C, D, S, W, Sigma);
    else if ( U && !W && !Sigma && power == 1)
        _U1_W0_Sig0_Pow1(result, nrow, ncol, C, D, S, W, U, Sigma);
    else if ( U && !W && !Sigma && power == 2)
        _U1_W0_Sig0_Pow2(result, nrow, ncol, C, D, S, W, U, Sigma);
    else if ( U && !W &&  Sigma && power == 1)
        _U1_W0_Sig1_Pow1(result, nrow, ncol, C, D, S, W, U, Sigma);
    else if ( U && !W &&  Sigma && power == 2)
        _U1_W0_Sig1_Pow2(result, nrow, ncol, C, D, S, W, U, Sigma);
    else if ( U &&  W && !Sigma && power == 1)
        _U1_W1_Sig0_Pow1(result, nrow, ncol, C, D, S, W, U, Sigma);
    else if ( U &&  W && !Sigma && power == 2)
        _U1_W1_Sig0_Pow2(result, nrow, ncol, C, D, S, W, U, Sigma);
    else if ( U &&  W &&  Sigma && power == 1)
        _U1_W1_Sig1_Pow1(result, nrow, ncol, C, D, S, W, U, Sigma);
    else if ( U &&  W &&  Sigma && power == 2)
        _U1_W1_Sig1_Pow2(result, nrow, ncol, C, D, S, W, U, Sigma);
}
