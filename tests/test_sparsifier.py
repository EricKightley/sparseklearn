...
#fHD = h5py.File('/home/eric/kmeansdata/fHD_temp.hdf5','a')
...
spr = Sparsifier(compression_target = 0.3)
spr.fit_sparsifier(X = X_train)
...
#gmm = GaussianMixture(compression = 1.0, n_components = 2, covariance_type = 'diag', precond = False)
#gmm.fit(X)
...
#s = Sparsifier(compression = .03, alpha = 0.01)
#s.fit_sparsifier(X_train)

#knn = KNeighborsClassifier(compression = 24, n_neighbors = 3, verbose = True)
#knn.fit(X_train, y_train)
#score = knn.score(X_test, y_test)
#print(score)


#kmc = KMeans(compression = 0.03, alpha = 0.5, n_clusters = 3)
#kmc.fit(X_train)

#spa = Sparsifier(compression=12, alpha = 4, verbose = True)
...
knn = KNeighborsClassifier(compression = 5, verbose = False, fHD = None, 
                           write_permission = False, precond = True, 
                           precond_X = True, dense_subsample = False,
...
#T1 = knn.HD_test(XT1)
#T2 = knn.apply_HD(XT2, knn.D_indices)
...
D = np.array([-1,1])
X_HDd = knn.apply_HD(X, D)
Y_HDd = knn.apply_HD(Y, D)
X_HDdmasked = knn.apply_mask(X_HDd, mask)
d1 = knn.pairwise_distances(X_HDdmasked,Y, mask = mask, D = D,
...
for j in range(nruns):
    ...
    X_train, y_train, X_test, y_test =  generate_mnist_dataset(f, n_train, n_test)
    knn = KNeighborsClassifier(compression = compressionV[0], verbose = False, fHD = fHD, 
                               write_permission = True, precond = True, 
                               precond_X = True, dense_subsample = False,
                               n_neighbors = n_neighbors, constant_subsample = False)
    knn.initialize(X_train)
    for i in range(len(compressionV)):
        knn = KNeighborsClassifier(compression = compressionV[i], verbose = False, fHD = fHD, 
                                   write_permission = False, precond = True, 
                                   precond_X = False, dense_subsample = False,
